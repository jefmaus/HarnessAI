...
1. **Verificar Estado del Arnés y Visión de Producto:** Consulta `harness/config.json`. Si `"configured": false`, no inventes stacks ni asumas rutas; solicita al usuario inicializar el arnés con: *"Configura el arnés siguiendo harness/SETUP_HARNESS.md"*. Si está configurado, internaliza la **Visión del Producto y Dominio del Negocio (Sección 1)** antes de cualquier interacción para asegurar alineación estratégica.
...
3. **Inspeccionar Foco Activo:** Comprueba si existe una carpeta en `harness/specs/active/`.
   ...
   * Si no existe: Pregunta al usuario si desea co-crear una nueva spec con `harness/specs/TEMPLATE.md` o activar una feature existente desde `harness/specs/backlog/`. Al co-crear, sigue obligatoriamente el diálogo interactivo, la alineación con la Visión 360° y el CHECKPOINT de aprobación previa antes de escribir cualquier archivo.
...
## 1. Visión del Producto y Dominio del Negocio (Visión 360°)
> [POR CONFIGURAR VÍA harness/SETUP_HARNESS.md]

* **Propósito General:** [Qué problema crítico resuelve el sistema y cuál es su propuesta de valor principal]
* **Usuarios y Actores Clave:** [Quiénes interactúan con la solución y sus roles principales]
* **Entidades Troncales (Lenguaje Ubicuo):** [Catálogo de 3 a 5 conceptos o entidades fundamentales del negocio]
* **Límites del Alcance (Scope Boundaries):**
  * **In-Scope (Dentro del Alcance):** [Capacidades y funcionalidades núcleo que forman parte de la solución]
  * **Out-of-Scope (Fuera del Alcance):** [Aspectos, integraciones o funciones explícitamente excluidas para evitar dispersión y feature creep]

---

## 2. Topología del Proyecto y Stack Tecnológico
> [POR CONFIGURAR VÍA harness/SETUP_HARNESS.md]
...
## 3. Máquina de Estados de Especificaciones (SDD)
...
* **Co-Creación de Specs y Alineación de Producto:** Proceso conversacional obligatorio gobernado por `harness/specs/TEMPLATE.md`. Jamás se debe generar el archivo físico `spec.md` en un solo turno asumiendo requisitos; requiere validar la pertinencia contra la **Visión del Producto (Sección 1)**, verificar que respete los límites *In-Scope*, y obtener la aprobación explícita del usuario sobre los contratos técnicos o invariantes y criterios de aceptación en el chat.
...
## 5. Política de Memoria Técnica JIT (`harness/memory/`)
...
## 6. Deny-List (Acciones Prohibidas)
...
* Proponer, crear o implementar funcionalidades que contradigan los límites *Out-of-Scope* o desvíen el foco del *Propósito General* establecido en la Visión del Producto (Sección 1) sin aprobación explícita del usuario.
* Introducir entidades o modelos de datos que rompan o dupliquen el Lenguaje Ubicuo troncal registrado en la Visión del Producto.